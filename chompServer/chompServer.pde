//CHOMP- SERVER -- run first (before client)
...
void draw() {  
  ...
  // if client is not null show food
  if ( client !=null) { 
    food();
  }
  serverData();
...
void serverData() {
  ...
  // if the client is available, import oponents score
  ...
  if (client != null) {
    ...
    // if oponents score is equal or greater than 10, display a LOSE screen
...
void food() {
  ...
  if (found) {
    ...
    //println("xposition", (0-posePosition.x)+width, (0-posePosition.y)+height);
  }//close found
}//close food

void winScreen() {
  // if players score is greater than 8, display a WIN screen
  // and stop the movement of all food/ non-food
  if (score>=8) {
    fill(#7CED41);
    rect(width/2, height/2, width, height);
